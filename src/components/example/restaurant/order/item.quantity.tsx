...
const ItemQuantity = (props: IProps) => {
    ...
    const handlePressItem = (item: IMenuItem, action: "MINUS" | "PLUS") => {
        ...
        if (item.options.length && isModal === false) {
            router.navigate({
                pathname: action === 'PLUS' ? "/product/create.model" : "/product/update.model",
    ...
    return (
        <ItemSingle menuItem={menuItem} handlePressItem={handlePressItem} showMinus={showMinus} quantity={quantity}></ItemSingle>
    )
}
